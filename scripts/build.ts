...
switch (platform) {
  case "win32":
    await $`
        scripts/build.bat
        cp webview/build/core/Release/webview.dll build/webview.dll
        `;
    break;
  case "linux":
    await $`
        ...
        cp build/core/Release/libwebview.so ../build/webview-${arch}.so
  ...
  case "darwin":
    await $`
        ...
        cmake -G "Ninja Multi-Config" -B build -S . \
            ...
            -DWEBVIEW_BUILD_TESTS=false
        cmake --build build --config Release
        cp build/core/Release/libwebview.dylib ../build/webview.dylib
